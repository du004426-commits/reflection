...
export default function Home() {
  ...
  function createProject(event: FormEvent) {
    ...
    const project: Project = { id: projectId, name, startedAt: new Date().toISOString(), status: "active", chatgptProjectUrl: projectLink.trim() || undefined, reflectionCadence: cadence, nextCheckIn: reminder.dueAt };
    setStore({ ...currentStore, projects: [project, ...currentStore.projects], reminders: [reminder, ...currentStore.reminders] }); setProjectName(""); setProjectLink("");
  }
  ...
  return <main className="shell">
    ...
    {!activeProject ? <>
      <section className="card"><h2>开始跟进一个项目</h2><p className="muted">可附上你的 ChatGPT 项目链接；目前会保存链接供你跳转，不能自动读取其中的聊天内容。</p><form onSubmit={createProject} className="project-form"><input value={projectName} onChange={(e) => setProjectName(e.target.value)} placeholder="项目名称，例如：新网站上线" aria-label="项目名称"/><input value={projectLink} onChange={(e) => setProjectLink(e.target.value)} placeholder="可选：粘贴 ChatGPT 项目链接" aria-label="ChatGPT 项目链接" type="url"/><div className="row"><select value={cadence} onChange={(e) => setCadence(e.target.value as ReflectionCadence)} aria-label="反思频率"><option value="daily">每天一次短 Check-in</option><option value="weekly">每周一次深度反思</option><option value="biweekly">每两周一次深度反思</option></select><button>开始跟进</button></div></form></section>
